  ...
  return (
    <motion.div drag dragConstraints={reference} whileDrag={{scale:1.1}} className='relative flex-shrink-0 w-60 h-72 rounded-[45px] bg-zinc-900/90 text-white px-8 py-10 overflow-hidden'>
        ...
        <div className='footer absolute bottom-0 w-full  left-0 '>
          ...
          { 
          data.tag.isOpen && (
             <div className={`tag w-full py-4 ${data.tag.tagColor === "sky" ? "bg-sky-500" : "bg-green-500"} flex items-center justify-center`}>
             <h3 className='text-sm text-black font-semibold'>{data.tag.tagTitle}</h3>
             </div> 
          )  }
